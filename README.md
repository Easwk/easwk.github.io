...
拿上例的handleClick函数来说，其绑定可以有：
  ...
  constructor(props) {
       super(props);
       this.handleClick = this.handleClick.bind(this); //在构造函数中使用bind绑定
  }
  ```

  ```
 <div onClick={this.handleClick.bind(this)}></div> //在调用处使用bind来绑定
  ```

  ```
 <div onClick={()=>this.handleClick()}></div> //在调用处通过arrow function来绑定
  ```
